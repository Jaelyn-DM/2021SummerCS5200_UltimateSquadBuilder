...
function setupButtonClickEventHandler() {
  ...
  $("#update-customized-team-button").click(function () {
    ...
    updateCustomizedTeam(APP_STORAGE[CURRENT_TEAM_KEY])
      .then(result => {
        console.log("Team selection: " + result);
      }).then(() => {
  ...
  // create button
  $("#create-customized-team-button").click(function () {
    ...
    updateCustomizedTeam(APP_STORAGE[CURRENT_TEAM_KEY])
      .then(result => {
        ...
        $('#update-customized-team-alert').text('Your squad has been created.');
        $('#update-customized-team-alert').fadeIn(500).delay(1500).fadeOut(500);
      }).then(() => {
        $(this).removeAttr("disabled");
      });
...
function renderTeamPlayers(editable) {
  ...
  [...Array(11).keys()].forEach(index => {
    ...
    if (!editable) {
      $selectPlayerElement.prop('disabled', 'disabled');
    }
  });
}

function initializeCreatePage() {
  console.log("Rendering create customized team page...");
  APP_STORAGE[CURRENT_TEAM_KEY] = {
    team_id: -1, // using dummy team_id as placeholder
    user: {
